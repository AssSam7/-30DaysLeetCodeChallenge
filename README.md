...
**Approach:** Question is so tricky that it leads to the approach by calculating the sub array of our input and then conducting the sum of those sub array, finally finding out the maximum number in that sums array. Actually, question is to find the maximum sum of the sub array not finding out the sub arrays. This approach is of O(n) where the maximum sum is calculated and returned from the input. Due to ES6 max of an array can be calculate with so ease by using **__spread operator__**.
